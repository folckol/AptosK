...
from pancakeswap.constants import (
    COIN_INFO,
    NETWORKS_MODULES,
    RESOURCES_ACCOUNT,
    FEE_SCALE,
...
class PancakeSwapClient(RestClient):
    def __init__(self, node_url: str, tokens_mapping: dict, hex_private_key: str, num_executions: int):
        ...
        # self.my_account.set_proxy()
        self.tokens_mapping = tokens_mapping
        self.executions_left = num_executions
    ...
    def get_token_reserves(self, token_x: str, token_y: str) -> (float, float):
        resource_type = f"{NETWORKS_MODULES['ResourceType']}::TokenPairReserve<{self.tokens_mapping[token_x]}," \
                        f" {self.tokens_mapping[token_y]}>"
        data = self.account_resource(
            AccountAddress.from_hex(RESOURCES_ACCOUNT),
            resource_type,
        )["data"]
        coin_x_reserve_value = int(data["reserve_x"])
        coin_y_reserve_value = int(data["reserve_y"])
