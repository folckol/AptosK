...
from aptoswap.constants import (
    COIN_INFO,
    NETWORKS_MODULES,
    RESOURCES_ACCOUNT,
    FEE_SCALE,
...
class AptoSwap(RestClient):
    ...
    def get_token_reserves(self, token_x: str, token_y: str) -> (float, float):
        resource_type = f"{NETWORKS_MODULES['ResourceType']}::Pool<{self.tokens_mapping[token_x]}," \
                        f" {self.tokens_mapping[token_y]}>"
        data = self.account_resource(
            AccountAddress.from_hex(RESOURCES_ACCOUNT),
            resource_type,
        )["data"]
        coin_x_reserve_value = int(data["x"]["value"])
        coin_y_reserve_value = int(data["y"]["value"])
